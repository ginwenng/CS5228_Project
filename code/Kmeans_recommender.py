...
class KMeansRecommender:
    ...
    """
    Use Kmeans algorithm to find cluster of input listing. Select k recommendations from cluster.

    """
    
    def __init__(self, num_clusters = 24):
    ...
    def get_overall_alignment_score(self, full_df):
        ...
        kmeans.fit(self.X_to_compare)

        for i in range(num_records):

            # get index of all rows in the same cluster as current row
            target_cluster = kmeans.labels_[i]
            idx_same_cluster = np.where(kmeans.labels_ == target_cluster)[0]
    ...
    def transform(self, idx, k=10, verbose=False):
        ...
        target_cluster = kmeans.labels_[idx]
        if verbose:
        ...
        recommendations_idx = np.random.choice(idx_same_cluster, k, replace=False)
        ...
        if verbose:
            ...
            pct_correct_planning_area = len(recommended_df[recommended_df["planning_area"] == original_planning_area]) / 10 * 100
            print(f"planning_area (same): {pct_correct_planning_area}%")

            original_subzone= original_X["subzone"]
            pct_correct_subzone = len(recommended_df[recommended_df["subzone"] == original_subzone]) / 10 * 100
            ...
            for feature in stats_features:
                original = original_X[feature]
                pct_correct = len(recommended_df[recommended_df[feature]==original])/ 10 *100
                print(f"{feature} (same): {pct_correct}%")


        return recommended_df_out